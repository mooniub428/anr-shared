...
%
function [Volume] = GetFullVolumeVectorField(LocalPatchFlat, Triangles, E, orientation, Frames, spaceStep, timeStep)
...
for fi = Frames
    ...
    Volume.PrincipalAxes((counter -1) * numOfCentroids + 1 : counter * numOfCentroids, 3) = repmat(fi * timeStep, numOfCentroids, 1);
...
Volume.PrincipalAxes(:, 3) = Volume.PrincipalAxes(:, 3) - mean(Volume.PrincipalAxes(:, 3)) * ones(numOfCentroids * numOfFrames, 1);
